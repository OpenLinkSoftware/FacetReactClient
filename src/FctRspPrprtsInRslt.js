// A component to display the <fct:result type="properties-in"> element of a Facet service response.
...
export default class FctRspPrprtsInRslt extends React.Component {

  constructor(props) {
    super(props);
    if (props.qryResult["@type"] !== "properties-in")
  ...
  render() {
    ...
    if (this.props.qryResult && this.props.qryResult.row)
    {
      ...
      // Facet properties-in result/view column mappings
      const columnHeadings = ['?s1 is value of Attribute', '', 'Count']; // TO DO: Should be `?s${pos}`, pos = 1,2,..
      ...
      html = (
        <div>
        <span><em>properties-in/ FctRspPrprtsInRslt result:</em></span>
